    ...
    <div className="w-[70%]  flex flex-col justify-between  mt-25 relative ">
        ...
        <div className=" w-[50%]  flex flex-col gap-5 max-md:w-full max-md:mb-5 shrink-0">
          ...
          <div className="mt-5 flex gap-2 ">
            ...
            <button className="flex gap-2 items-center bg-main-hover-3 px-5 py-2 rounded-md justify-between duration-200 ease-in-out transition-all hover:bg-main-hover-5 cursor-pointer group">
              <span className="font-yekan text-main-hover-4 ">قیمت طلا</span>
      ...
      <div className="w-[95%] p-2  border-gray-300  self-center -translate-5 z-10 bg-gray-100  rounded-2xl grid grid-cols-4 gap-1 max-lg:grid-cols-2 max-lg:translate-0  py-5 ">
        {MainFrame.map((m, index) => (
          <div
            className="flex flex-col justify-center items-center font-yekan gap-1"
            key={index}
          >
            <h1 className="font-bold text-lg">{m.mainSec}</h1>
            <p className="max-md:text-sm text-gray-700 font-yekan">
              {m.secSec}
            </p>
          </div>
        ))}
